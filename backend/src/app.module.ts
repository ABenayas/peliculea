...
@Module({
  imports: [
    TypeOrmModule.forRoot({
      type: 'postgres',
      host: 'localhost', // Tiene que coincidir con el servicio de docker-compose, elhost, si uso Docker, si no, localhost.
      port: 5432,
      username: 'postgres',
      password: 'postgres',
      database: 'default',
      entities: [User], // Aquí metemos todas las entidades que tenemos. Aunque, parece que se puede hacer de manera automática así: 'dist/**/*.entity{.ts,.js}', si no, sería indicando las entidades una a una [User,...], e importándolas arriba.
      synchronize: true, // Aquí está la opción de no sincronizar automáticamente, false. Al principio lo dejo true.
